...
<form method="POST" class="container-add-account" action="{{ route('admin.update-specific-account', $user->id) }}" enctype="multipart/form-data">
    ...
    <div class="image">
        @if($user->has_image == 0)
        <img class="image-item" id="preview-image-before-upload" src="{{ asset('images/default.jpeg') }}" alt="{{ $user->name}}">
        @else
        <img class="image-item" id="preview-image-before-upload" src="{{ asset('images/avatars/'.$user->avatar) }}" alt="{{ $user->name}}">
        @endif
    ...
    <div class="forms">
        <div class="form-group-1">
            <div class="form-group first">
                ...
                <input id="userName" type="userName" class="form-control @error('userName') is-invalid @enderror" name="userName" autofocus placeholder="Enter new username" value="{{$user->userName}}">
            ...
            <div class="form-group second">
                ...
                <select readonly disabled="true" name="userType" id="userType" class="form-control">
                    <option value="{{$user->userType}}" disabled selected hidden>{{$user->userType}}</option>
